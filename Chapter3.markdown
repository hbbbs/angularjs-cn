...
Angular意识也意味着Angular知道所有的XHR何时向服务器放出, 从而可以避免页面加载所等待的间隔时间. 场景运行器知道何时加载一个页面, 从而比Selenium测试更具确定性, 例如, 超时等待页面加载时任务可能失败.
...
在JavaScript世界里, 编译通常意味着压缩代码, 虽然一些实际的编译可能使用的时Google的Closure库. 但是为什么你会希望将所有漂亮的, 写的很好, 很容易理解代码变得不可读呢?

原因之一是我们的目标是是应用程序更快的响应用户. 这是为什么客户端应用程序几年前不像现在腾飞得这么快的主要原因. 能够越早获取应用程序并运行, 响应得也越早.

这种快速响应是压缩JS代码的动机. 代码越小, 越能有效的减小负载, 同时能够更快的将相关文件发送给用户. 这在移动应用程序中显得尤为重要, 因为其规模会成为瓶颈.

这里有几种方法可以压缩你给应用程序所编写的AngularJS代码, 每种方法都具有不同的效果.

**基本的和简单的优化**

这包括压缩所有在你的代码中使用的变量, 但是不会压缩属性. 这就是所谓的传递给Closure Compiler的简单优化.

这不会给你带来多大的文件大小的节省, 但是你仍然可以获得一个可观的最小开销.
...
高级优化有一点棘手, 它会试图重名几乎任何东西和每个函数. 得到这个级别的优化工作, 你将需要通过显示的告诉它哪些函数, 变量和属性需要重命名(通过使用一个externsfile). 这通常是通过模板访问函数和属性.

编译将会使用这个`externs`文件, 然后重命名所有的东西. 如果处理好, 这可能会导致的你的JavaScript文件大幅度的减小, 但是它的确需要相当大的工作量, 包括每次改变代码都要更新externs文件.
...
一旦所有的变量都模糊处理或压缩,上面列出的两种写法 大概是 找出AngularJS服务或变量最初命名 的 唯一方法了。

> 每次都是数组的方式注入是比较好的处理方式, 以避免开始编译代码时的错误. 挠头并试图找出为什么提供的$e变量丢失了(一些任务的混肴版本压缩了它)是不值得的.
...
+ "暂停所有异常"是内置在当今大多数开发者工具中的一个非常有用的选项. 当发现一个异常时调试器会终止继续运行并高亮显示导致异常的代码行.
